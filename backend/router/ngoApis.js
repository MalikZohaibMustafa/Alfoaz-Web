...
ngoApis.post("/register", upload.single("certificate"), async (req, res) => {
  try {
    ...
    const certificate = `http://localhost:3001/profile/${req.file.filename}`;
  ...
  try {
    ...
    if (req.files) {
      ...
      if (req.files.coverImage) {
        updateObj.coverImage = `http://localhost:3001/profile/${req.files.coverImage[0].filename}`;
      }
      if (req.files.profileImage) {
        updateObj.profileImage = `http://localhost:3001/profile/${req.files.profileImage[0].filename}`;
      }
      if (req.files.certificate) {
        updateObj.certificate = `http://localhost:3001/profile/${req.files.certificate[0].filename}`;
      }
